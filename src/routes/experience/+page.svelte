...
<div class="grid grid-cols-8 md:grid-cols-9 gap-4 -mt-12 lg:mt-20">
    ...
    <div class="col-span-8 md:col-span-2 grid grid-flow-col md:grid-flow-row place-content-center md:place-content-start" >
    ...
    <div class="col-span-8 md:col-span-5 prose font-mono mt-10 md:mt-0" >
        <h3 class="text-white"><span class="text-amber-300 md:hidden inline-flex">0{$WorkStore[selectedWorkplace].id}.</span> {$WorkStore[selectedWorkplace].title} <span class="text-amber-300 hidden md:inline-flex">@{$WorkStore[selectedWorkplace].company}</span></h3>
        <h4 class="text-gray-500">{$WorkStore[selectedWorkplace].from} - {$WorkStore[selectedWorkplace].to}</h4>
